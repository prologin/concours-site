...
def parse_xml(s, problem_props):
    ...
    for test in xml.findall('test'):
        t = TestResult()
        if 'performance' in problem_props and (test.attrib['id'] in
        problem_props['performance'].split()):
        ...
        if test.find('summary').attrib['error'] == '0':
            ...
            if details.find('diff') is not None:
                ...
                t.details = (details.find('program').text,
                        details.find('ref').text)
        if test.find('debug'):
            t.debug = test.find('debug').text
        else:
            t.debug = None
        if 'hidden' in problem_props and (test.attrib['id'] in
        problem_props['hidden'].split()):
            t.hidden = True
