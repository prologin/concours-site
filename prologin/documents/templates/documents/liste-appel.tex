...
\begin{center}
    \Large
    Liste des candidats -- Prologin {{ year }} -- {{ center.name|escapetex }}
\end{center}
...
{% for wish in wishes %}{% with user=wish.contestant.user %}
{{ user.last_name|escapetex }} & {{ user.first_name|escapetex }} & {{ user.birthday|date:"Y-m-d"|escapetex }} & & \\\hline
{% endwith %}{% endfor %}
