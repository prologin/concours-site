...
class InlineGrammar(mistune.InlineGrammar):
    ...
    mention = re.compile(r'^@(?:(?P<username>[^\s]+)|\{(?P<full_username>.+?)\})')
    text = re.compile(r'^[\s\S]+?(?=[\\<!\[_*`:@~]|https?://| {2,}\n|$)')


class InlineLexer(mistune.InlineLexer):
    default_rules = ['emoji', 'mention'] + mistune.InlineLexer.default_rules
    ...
    def output_mention(self, m):
        username = m.group('username') or m.group('full_username')
        ...
        # Mentions limiter
        if self._mention_count >= settings.FORUM_MENTIONS_PER_MESSAGE:
            return m.group(0)
